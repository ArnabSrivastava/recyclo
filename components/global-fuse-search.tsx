...
export function GlobalFuseSearch({ isOpen, onClose }: GlobalFuseSearchProps) {
  ...
  const filteredProductResults = useMemo(() => {
    const isMenQuery = /\b(male|men|mens|men's|man|gents|boy|boys)\b/i.test(query);
    const isWomenQuery = /\b(female|women|womens|women's|woman|lady|ladies|girl|girls)\b/i.test(query);
  ...
  const POPULAR_SEARCHES = ['Adidas Tracksuit', 'Nike Hoodie', 'Puma Joggers', 'Zara Silk', "Levi's Denim", 'Banarasi Silk'];
      ...
      <div className="relative w-full max-w-2xl bg-card border-2 border-foreground shadow-2xl rounded-none overflow-hidden animate-in zoom-in-95">
        <form onSubmit={handleSearchSubmit} className="relative border-b-2 border-border bg-muted/20 p-4 flex items-center gap-3">
          ...
          <input
            type="text"
            autoFocus
            placeholder="Search products, brands (Nike, Adidas, Zara), materials..."
            value={query}
        ...
        {hasQuery && (
          <div className="flex items-center gap-2 px-4 py-2 bg-muted/40 border-b border-border text-xs">
            ...
            >
              All ({filteredProductResults.length + itemResults.length})
            </Button>
            <Button
              ...
              className="h-7 px-3 text-xs font-bold rounded-none cursor-pointer"
            >
              Upcycled Products ({filteredProductResults.length})
            </Button>
          </div>
        )}
        ...
        <div className="max-h-[60vh] overflow-y-auto p-6 space-y-4">
          {!hasQuery ? (
          ...
          ) : filteredProductResults.length === 0 && itemResults.length === 0 ? (
            ...
            <div className="space-y-4">
              {(activeTab === 'ALL' || activeTab === 'PRODUCTS') && filteredProductResults.length > 0 && (
                <div className="space-y-2">
                  <div className="text-xs font-extrabold uppercase tracking-wider text-muted-foreground flex items-center justify-between">
                    ...
                    <span>{filteredProductResults.length} matches</span>
                  </div>
